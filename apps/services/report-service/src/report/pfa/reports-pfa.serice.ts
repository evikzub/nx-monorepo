...
} from './report-pfa-values.service';
...
interface ReportPFAProps {
  ...
  reportTitle: string;
  reportDate: string;
  commonContext: any; // TODO: define a schema
  clientProps: {
...
export class ReportsPFAService {
  ...
  private getReportParams(config: ConfigProps): ReportPFAProps {
    ...
    this.logger.log(
      `Logo file path: ${path.join(config.rptLogoPath, logoFile)}`,
    );

    //const commonContext = pfaCommonData; //this.getReportContext(config.rptJsonPath);

    return {
      css_file: path.join(config.rptCSSPath, 'pdf.css'),
  ...
  async createReportPFA(config: ConfigProps) {
    ...
    try {
      ...
      fs.writeFileSync(`${config.rptOutputPath}/output.html`, html);
      // Convert HTML to PDF
...
//      executablePath: '/usr/bin/chromium', // Adjust path based on your image
    ...
    const baseUrl = `file://${path.resolve(config.rptLogoPath, '..')}`;
